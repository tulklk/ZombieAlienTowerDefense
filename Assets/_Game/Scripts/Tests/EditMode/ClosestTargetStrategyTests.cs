...
{
    public class ClosestTargetStrategyTests
    {
        ...
        private EnemyController CreateEnemy(EnemyRegistry registry, Vector3 position)
        {
        ...
        [Test]
        public void SelectTarget_PicksSmallestXZDistance()
        {
            var registry = new EnemyRegistry();
            EnemyController far = CreateEnemy(registry, new Vector3(4f, 0f, 0f));
            EnemyController near = CreateEnemy(registry, new Vector3(1f, 0f, 0f));

            var strategy = new ClosestTargetStrategy();
        ...
        [Test]
        public void SelectTarget_IgnoresVerticalDistance()
        {
            var registry = new EnemyRegistry();
            EnemyController closeXZ = CreateEnemy(registry, new Vector3(1f, 100f, 0f));
            EnemyController fartherXZ = CreateEnemy(registry, new Vector3(3f, 0f, 0f));

            var strategy = new ClosestTargetStrategy();
            EnemyController result = strategy.SelectTarget(registry, Vector3.zero, 5f);

            Assert.AreEqual(closeXZ, result);
            Assert.AreNotEqual(fartherXZ, result);
        }
        ...
        {
            var registry = new EnemyRegistry();
            CreateEnemy(registry, new Vector3(50f, 0f, 0f));
